...
public class UsrMemberController {
	...
	public ResultData login(String loginId, String loginPw) {
		...
		ResultData loginMember = memberService.login(loginId, loginPw);
		return loginMember;
	}
	...
	public ResultData doJoin(String loginId, String loginPw, String name, String nickname, String cellphoneNum,
		...
		if (id == -1) {
			return ResultData.from("F-2", Ut.f("이미 사용중인 아이디(%s)입니다.", loginId));
		}if (id == -2) {
			return ResultData.from("F-3", Ut.f("이미 사용중인 이름(%s)과 이메일(%s)입니다.", name,email));
		}
